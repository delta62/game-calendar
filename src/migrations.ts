...
export function migrate(): void {
    ...
    let migratedMonths: Month[] = storageState.map((oldState: any) => ({
        ...
        finishDate: oldState.completeDate && new Date(oldState.completeDate).getTime(),
        // Change "hundredPercentDate" to "completeDate"
        completeDate: oldState.hundredPercentDate && new Date(oldState.hundredPercentDate).getTime(),
    }))
