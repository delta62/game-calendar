...
function updateYear(
  ...
  year: number,
  month: number,
  replacement: Partial<Month>
): State
function updateYear(
  state: State,
  year: number,
  month: number,
  replacement: (oldVal: Month) => Month
): State
function updateYear(
  state: State,
  year: number,
...
function months(state: State = {}, action: Action): Record<number, Month[]> {
  switch (action.type) {
    ...
    case "START_TOGGLED":
      return updateYear(state, action.year, action.month, (last) => ({
        startDate: last.startDate ? undefined : action.time,
      }))
    case "FINISH_TOGGLED":
      return updateYear(state, action.year, action.month, (last) => ({
        finishDate: last.finishDate ? undefined : action.time,
      }))
    case "COMPLETE_TOGGLED":
      return updateYear(state, action.year, action.month, (last) => ({
        completeDate: last.completeDate ? undefined : action.time,
      }))
    default:
      return state
  }
